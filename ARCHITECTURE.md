...
| `PriorityQueue/` | Priority Queue | Portfolio-wide ranking of every community by vacancy rate, plus two derived-signal sections: the "Fast-Track Approvals" callout and the "Aging 30+ Days" column — see [Aging flag & Fast-Track Approvals](#aging-flag--fast-track-approvals-callout) below. An "As of" date picker (defaults to today) drives both `usePriorityQueue` and `useFastTrackUnits`, showing each community's most recent report at or before the chosen date — reports are weekly, so this is "whichever week was current then," not per-day granularity |
