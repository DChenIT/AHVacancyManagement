...
**Dormant leftover:** `cr1e9_communities` also has `cr1e9_regionalmaintenancesupervisor` and `cr1e9_compliancespecialist` columns, and `scripts/import-communities-csv.ps1` still maps them — both added during a brief attempt to replace this SharePoint approach with a CSV import instead (see `memory-bank.md`), abandoned once the live connector came back into scope. Nothing in the app currently reads those two columns; they're harmless to leave in place rather than deleting (see the Turn Status saga elsewhere in `memory-bank.md` for why repeatedly deleting/recreating Dataverse columns is worth avoiding).
