...
- **Every report save creates a brand-new `VacancyReport` + its `UnitUpdate` rows** — there is no edit-existing-report flow. This matches an intentional "each save is a new weekly snapshot" model, which keeps full history naturally without needing a diff/reconcile step, and is also what makes the aging-streak and Fast-Track features possible (they read that history).
